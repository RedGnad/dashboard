...
---
## 2bis. Priorités Clés (révisées)
- Swarm inference (OpenGradient) via InferenceProvider pluggable (stub d’abord), avec `inferenceProofHash` (audit + pack v2)
- Oracle de prix (Switchboard) pour spot/twap + quantization stable (6 décimales), fallback synthétique, exposition `priceQuantized`
- Double publication v2/v3 des hashes features (FEATURE_SET_VERSION=3 derrière flag) pour intégrer momentum/priceQuantized sans casser v2
...
| Open Gradient (Swarm Inference) | 🔄 provider abstrait prioritaire (stub OG + `inferenceProofHash`) |
...
| Switchboard Oracle (prix spot) | 🔄 intégrer (quantization + fallback + `priceQuantized`) |
...
1. (M1) Abstraction `InferenceProvider` (local vs open-gradient). Critères: sélection via env, sortie stable, métriques latence.
2. (M1) Intégrer Open Gradient (OG) stub + `inferenceProofHash` dans audit. Critères: test re-hash blob OK; endpoint provider OK.
3. (M2) `strategy-model.json` reste fallback local + `modelHash` (coexiste OG).
5. (Future) Multi-actif : sélection du meilleur candidat (score max).

### B. Features & Données
6. (M1) Intégrer Switchboard (prix spot) → `priceQuantized` (exp_* d’abord). Critères: fallback synthétique, quantization 6 déc., test stabilité multi-run.
7. (M2) Générer features de base :
   - `balanceStableRatio`
   - `balanceTargetRatio`
   - `allocationDeviation`
...
8. (M2) Calculer `featureHash` (ordre stable, JSON canonique -> keccak256).
...
13. (M3) Proof Pack v2: ajouter `inference.json` + vérif CLI (reconstruction pré-image). Critères: verify:proof-pack passe.
14. (M4) Endpoint `/api/strategy/replay/:rollingHash` :
    - Recharge décision
    - Recompose features
...
20. (M6) Registry simple : `models/` + alias `current` + pointer OG model metadata (weightsCommit, codeHash).
...
|-----------|---------|-----------------|
| M1 | InferenceProvider + OG stub + inferenceProofHash | Preuve d’inférence distribuée, prêt pour pack v2 |
| M2 | Switchboard spot price + quantization | Features stables + readiness v3 |
| M3 | Feature set v3 (momentum+priceQuantized) double publication | Hashing enrichi sans casser v2 |
