...
export interface DelegationAuditEntryV1 {
  ...
  // Version of the inference provider/model pipeline
  ...
  // Hash of the canonical inference proof blob (binds provider+version+modelHash+featureHashV2+delegator+ts)
...
export function buildAuditEntry(base: Partial<DelegationAuditEntryV1>): DelegationAuditEntryV1 {
  ...
  guardrailReasonsAll: Array.isArray((base as any).guardrailReasonsAll) ? (base as any).guardrailReasonsAll : undefined,
    ...
    inferenceVersion: base.inferenceVersion,
    ...
    inferenceProofHash: base.inferenceProofHash,
...
export function appendAudit(entry: Partial<DelegationAuditEntryV1>) {
  ...
  if (base.inferenceVersion !== undefined) finalObj.inferenceVersion = base.inferenceVersion
  ...
  if (base.inferenceProofHash !== undefined) finalObj.inferenceProofHash = base.inferenceProofHash
    try {
