// @ts-nocheck
import { UniswapV2PairInfo, SwapEvent } from "generated";
...
// Generic UniswapV2 Factory: PairCreated(address token0, address token1, address pair, uint)
...
// Generic Pair: Swap(address indexed sender, uint amount0In, uint amount1In, uint amount0Out, uint amount1Out, address indexed to)
  ...
  Swap: {
    handler: async ({ event, context }: any) => {
      try {
        ...
        // Determine tokenIn / tokenOut and amounts
        ...
        const id = `${event.chainId}_${event.block.number}_${event.logIndex}_v2`;
