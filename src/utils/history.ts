...
export type DcaRunEvent = {
  ...
  delegator: string
  amountInUSDC?: string
  // If the run was skipped (AI gating or guardrails), amountInUSDC should be '0'.
  // amountIntendedUSDC can record the pre-gated intended spend for analysis/UX.
  ...
  ai?: { actionType?: string; rawScore?: number; momentum?: number }
  sizing?: { basePct?: number; effectivePct?: number }
  // Telemetry for UI baselines: record the USDC balance snapshot and a manual baseline
  // computed as sizePctBase * balanceAtRun (bounded by min/max if configured).
  balanceAtRunUSDC?: string
