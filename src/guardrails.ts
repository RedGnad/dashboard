...
export interface GuardrailsConfig {
  ...
  // Hash checks control: prefer stable (v2) and optionally ignore v1 (timestamp-based)
  checkFeatureHashV1?: boolean
  checkFeatureHashV2?: boolean
}
...
const DEFAULTS: GuardrailsConfig = {
  ...
  checkFeatureHashV1: false,
  checkFeatureHashV2: true,
}
...
export function evaluateGuardrailsV2(gr: GuardrailsConfig, inputs: EvaluateV2Inputs): GuardrailEvaluation {
  ...
  if (gr.checkFeatureHashV1 !== false && feat.featureHash && inputs.ctx.lastDecisionFeatureHash && feat.featureHash !== inputs.ctx.lastDecisionFeatureHash) {
    warnings.push('feature_hash_mismatch')
    if (gr.hashMismatchHardBlock) { blocked = true; reason = reason || 'feature_hash_mismatch' }
  }
  if (gr.checkFeatureHashV2 !== false && feat.featureHashV2 && inputs.ctx.lastDecisionFeatureHashV2 && feat.featureHashV2 !== inputs.ctx.lastDecisionFeatureHashV2) {
