...
| Inference proof hash | Blob canonique d’inférence (`inference.json`) → `inferenceProofHash` | `verify:proof-pack` + re-hash blob |
| Data provenance (prix) | `priceQuantized` issu d’oracle (Switchboard) + fallback annoté | `verify:latest` / diff features |
...
5. Version 2: inclut `inference.json` (blob OG canonique) et fields `modelHash`, `inferenceProofHash` alignés.
