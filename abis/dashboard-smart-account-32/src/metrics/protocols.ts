...
export interface ProtocolDef {
  ...
  // Optional: Envio protocol identifier if different from our canonical id
  // Example: { id: 'ambient', envioId: 'ambient-core' }
  envioId?: string
}
...
function parseHours(hours?: any, def = 24): number {
  ...
  // Allow fractional hours for lightweight scans (e.g., 0.25 = 15 minutes)
  return Math.min(168, Math.max(0.01, n))
}
...
export async function computeDailyMetricsRPC(proto: ProtocolDef, opts: RpcDailyOpts = {}): Promise<DailyProtocolMetrics> {
  ...
  const maxBlocks = Math.max(50, Number(opts.maxBlocksScan ?? 1000))
  let start = fromBlock
  const span = Number(toBlock - fromBlock)
  if (span > maxBlocks) {
    start = toBlock - BigInt(maxBlocks)
  }
