...
| Inférence ouverte | Provider distribuable & vérifiable | `InferenceProvider` (local/OG), `inferenceProofHash` (audit + pack) |
| Prix oracle | Données spot externes et stables | Switchboard (Surge) + `priceQuantized` (quantization 6 déc., fallback)
...
| `GET /api/inference/provider` | Provider actif (local / OG stub) |
| `GET /api/strategy/features/head` | Features + `featureHash` / `featureHashV2` (+ v3: `priceQuantized`) |
