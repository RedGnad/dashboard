...
function FitToUnit({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const scale = 1.5 / maxSide; // fit nicely in view
...
function LookAtMouse({ target }: { target: React.RefObject<THREE.Group> }) {
  ...
  useFrame(() => {
    ...
    const yaw = mouse.current.x * 0.8; // left/right
    const pitch = -mouse.current.y * 0.5; // up/down
...
export default function ModelStage({ modelUrl = "/model.glb" }: { modelUrl?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // HEAD may fail on some servers, fallback to GET without reading body
        let r = await fetch(modelUrl, { method: "HEAD", cache: "no-store" });
        if (!r.ok) {
          // fallback GET to inspect content-type
        ...
        <group ref={root}>
          {hasModel ? (
            <Suspense fallback={<FallbackMesh />}> 
              {/* If GLB fails to load, Suspense will keep fallback; ensure the url exists in /web/public/model.glb */}
