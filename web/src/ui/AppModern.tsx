...
export default function AppModern() {
  ...
  async function refreshSaPanel() {
    try {
      ...
      try {
        ...
        // keep undefined if derivation fails; do not fall back to EOA (prevents wrong-core checks)
        delegatorSa = undefined;
      }
      ...
      if (!delegatorSa) {
        const prev = saPanel?.delegator?.address;
        if (prev) delegatorSa = prev;
        else if (address) delegatorSa = address;
      }
      ...
      const diag = await fetch(diagUrl.toString(), { cache: "no-store" }).then(
        (r) => r.json()
      );
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const r = await fetch(url.toString(), { cache: "no-store" }).then((x) =>
          x.json()
        );
        if (r && (r.ok || r.address)) {
  ...
  // Check for existing delegations when delegator SA address is known
  useEffect(() => {
    (async () => {
      const addr = saPanel?.delegator?.address;
      if (!addr) {
      ...
      try {
        const url = new URL(`/api/status/${addr.toLowerCase()}`, apiBase || window.location.origin);
        const r = await fetch(url.toString()).then((x) => x.json());
        if (r?.hasDelegation) {
          setHasDelegation(true);
          console.log('✅ Existing delegation detected');
        } else {
          setHasDelegation(false);
  ...
  refreshAllBalances.current = async () => {
    ...
    try {
      if (address) {
        ...
        const r2 = await fetch(url2.toString(), { cache: "no-store" }).then(
          (x) => x.json()
        );
  ...
  async function createAndPostDelegation() {
    ...
    try {
      ...
      if (res.status === 200 && res.body?.ok) {
        ...
        // Server returns 'immediate' when autostart is requested
        const immediate = res.body?.immediate || res.body?.immediateRun;
        if (immediate) {
          if (immediate.ok) {
            setMsg(
              (m) =>
                `${
                  m ? m + "\n" : ""
                }Premier swap exécuté immédiatement (hash: ${immediate.hash})`
            );
          } else {
            setMsg(
              (m) =>
                `${m ? m + "\n" : ""}Premier swap a échoué: ${immediate.error}`
            );
          }
        }
        setMsg((m) => `${m ? m + "\n" : ""}Delegation saved.`);
    ...
    try {
      ...
      const r = await fetch(`${apiBase || ""}/api/scheduler/start`, {
        ...
        body: JSON.stringify({
          ...
          immediate: true,
          unwrapToMon,
          // Start AI-driven scheduler so analytics panels populate
          jobType: "dca_ai",
          dynamicByAI: true,
          respectAiAction: true,
          provider: "openai",
      ...
      if (j?.ok && j?.job) {
        ...
        // Rafraîchir les soldes et panels immédiatement
        try {
          refreshAllBalances.current?.();
          refreshSaPanel();
        } catch {}
  ...
  // Direct swap function like in AppModern-archive.tsx
  async function directSwapToMon(tokenSymbol: string, amount: string) {
    if (!address || !publicClient || !walletClient || !saPanel.delegator?.address) return;
    
    try {
      setBusy(true);
      setMsg(`Swap direct ${tokenSymbol} → WMON...`);
      
      // Get token info
      const tokenInfo = tokensMeta[tokenSymbol];
      ...
      // Setup environment and smart account
      const env = getDeleGatorEnvironment(10143);
      const smart = await toMetaMaskSmartAccount({
        client: publicClient,
        implementation: Implementation.Hybrid,
      ...
      const bundlerClient = createBundlerClient({
        client: publicClient,
        transport: http(
          "https://rpc.zerodev.app/api/v2/bundler/7535339b-9ae3-41bb-ad97-5375bae4a51b"
        ),
      });
      ...
      const swapCalldata: Hex = encodeFunctionData({
        ...
        functionName: "swapExactTokensForTokens",
        args: [amt, 0n, [tokenAddress, WMON], smart.address as Address, deadline],
      });
      
      // Send UserOp directly (like AppModern-archive.tsx)
      const gasPrice = await publicClient.getGasPrice();
      const maxFeePerGas = gasPrice * 2n; // Higher gas to avoid mempool issues
      const maxPriorityFeePerGas = maxFeePerGas / 2n;
      
      const hash = await bundlerClient.sendUserOperation({
        account: smart,
        calls: [
          { to: tokenAddress, value: 0n, data: approveCalldata },
          { to: UNISWAP_V2_ROUTER02, value: 0n, data: swapCalldata },
        ],
        maxFeePerGas,
        maxPriorityFeePerGas,
      });
      
      setMsg(`✅ Swap envoyé! UserOp: ${hash}`);
      ...
      try {
        ...
        if (receipt.success) {
          setMsg(`🎉 Swap confirmé! TX: ${receipt.receipt.transactionHash}`);
          // Refresh balances
          setTimeout(() => refreshAllBalances.current?.(), 2000);
        } else {
          setMsg(`❌ Swap échoué on-chain`);
        }
      } catch {
        setMsg(`⏳ Swap en cours... (vérifiez l'explorer)`);
      }
      
    } catch (e: any) {
      setMsg(`❌ Swap échoué: ${e?.message || e}`);
    } finally {
      setBusy(false);
    }
  }

  // Direct unwrap function
  async function directUnwrapWmon() {
    if (!address || !publicClient || !walletClient || !saPanel.delegator?.address) return;
    
    try {
      setBusy(true);
      setMsg("Unwrap WMON → MON...");
      
      // Get WMON balance
      const wmonBalance = allBalances?.tokens?.['WMON'] || '0';
      const wmonAmount = BigInt(wmonBalance);
      
      if (wmonAmount === 0n) {
        setMsg("Aucun WMON à unwrap");
        return;
      }
      ...
      // Build unwrap call
      const unwrapCalldata: Hex = encodeFunctionData({
        abi: [
          {
            name: "withdraw",
            type: "function",
      ...
      const hash = await bundlerClient.sendUserOperation({
        account: smart,
        calls: [{ to: WMON, value: 0n, data: unwrapCalldata }],
        maxFeePerGas,
        maxPriorityFeePerGas,
      });
      
      setMsg(`✅ Unwrap envoyé! UserOp: ${hash}`);
      ...
      try {
        ...
        if (receipt.success) {
          setMsg(`🎉 Unwrap confirmé! TX: ${receipt.receipt.transactionHash}`);
          setTimeout(() => refreshAllBalances.current?.(), 2000);
        } else {
          setMsg(`❌ Unwrap échoué on-chain`);
        }
      } catch {
        setMsg(`⏳ Unwrap en cours...`);
      }
      
    } catch (e: any) {
      setMsg(`❌ Unwrap échoué: ${e?.message || e}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function convertAllToMon() {
    ...
    const tokensToConvert = Object.entries(allBalances?.tokens || {})
      .filter(([sym, balance]) => {
        if (sym === 'WMON') return false; // Skip WMON
        const num = Number(balance || '0');
        return num > 0;
      });
    
    if (tokensToConvert.length === 0) {
      setMsg("Aucun token à convertir");
      return;
    }
    
    try {
      setBusy(true);
      setMsg(`🔄 Étape 1: Conversion ${tokensToConvert.length} tokens → WMON...`);
      
      // Convert each token one by one to avoid nonce conflicts
      for (const [symbol, balance] of tokensToConvert) {
        const tokenInfo = tokensMeta[symbol];
        if (!tokenInfo) continue;
        
        const decimals = tokenInfo.decimals || 18;
        const amount = (Number(balance) / Math.pow(10, decimals)).toString();
        
        setMsg(`Conversion ${symbol}: ${amount}...`);
        await directSwapToMon(symbol, amount);
        
        // Wait between swaps to avoid nonce conflicts
        await new Promise(r => setTimeout(r, 3000));
      }
      
      setMsg(`🔄 Étape 2: Unwrap WMON → MON natif...`);
      await new Promise(r => setTimeout(r, 2000)); // Wait for balances to update
      await refreshAllBalances.current?.(); // Refresh to get new WMON balance
      await directUnwrapWmon();
      
      setMsg(`✅ Conversion complète! ${tokensToConvert.length} tokens → MON natif`);
  ...
  async function directTopupUsdc() {
    ...
    try {
      ...
      try {
        if (publicClient) {
          await publicClient.waitForTransactionReceipt({ hash: tx as Hex });
        }
      } catch {}
      setMsg(`Top-up direct USDC confirmé: ${tx} (${topupAmountNum} USDC)`);
      try {
        await refreshAllBalances.current?.();
      } catch {}
      try {
        await refreshSaPanel();
      } catch {}
    } catch (e: any) {
  ...
  const calculateEOAProgress = () => {
    ...
    const saUsdc =
      Number(allBalances?.tokens?.USDC ?? saPanel?.delegator?.usdc ?? "0") /
      1e6;
    const saMon =
      Number(allBalances?.MON ?? saPanel?.delegator?.mon ?? "0") / 1e18;
    const saWmon =
      Number(allBalances?.tokens?.WMON ?? saPanel?.delegator?.wmon ?? "0") /
      1e18;
      ...
      <div className="absolute top-6 left-6 space-y-3 z-30">
        ...
        <BalanceBar
          ...
          balances={{
            USDC: fmtToken(
              "USDC",
              allBalances?.tokens?.USDC ?? saPanel?.delegator?.usdc
            ),
            MON: fmtToken("MON", allBalances?.MON ?? saPanel?.delegator?.mon),
            WMON: fmtToken(
              "WMON",
              allBalances?.tokens?.WMON ?? saPanel?.delegator?.wmon
            ),
          }}
        ...
        <div className="w-[380px] backdrop-blur-md bg-white/10 border border-white/20 rounded-2xl shadow-lg p-2">
          <div className="text-white/90 font-semibold mb-1 text-sm">
            Other tokens
          </div>
              ...
              <div className="flex flex-wrap gap-1 text-[11px]">
                {Object.entries(eoaBalances?.tokens || {})
                  ...
                  .map(([sym, val]) => (
                    <div
                      key={`eoa-${sym}`}
                      className="px-1.5 py-1 rounded bg-white/5 border border-white/10 text-white/90"
                    >
                      {sym}:{" "}
                      <span className="font-mono">{fmtToken(sym, val)}</span>
                    </div>
                  ))}
                {(!eoaBalances?.tokens ||
                  Object.keys(eoaBalances.tokens).filter(
                    (k) => !["USDC", "WMON"].includes(k)
                  ).length === 0) && <div className="text-white/50">—</div>}
              </div>
            </div>
            <div>
              <div className="text-white/70 text-[11px] mb-1">
                Smart Account tokens
              </div>
              <div className="flex flex-wrap gap-1 text-[11px]">
                ...
                {(!allBalances?.tokens ||
                  Object.keys(allBalances.tokens).filter(
                    (k) => !["USDC", "WMON"].includes(k)
                  ).length === 0) && <div className="text-white/50">—</div>}
              </div>
            </div>
