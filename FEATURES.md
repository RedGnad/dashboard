...
| Oracle Integration (Switchboard) | External price feed + safe fallback | 🔄 | Adapter en place (Surge), activer ENABLE_SWITCHBOARD, quantization 6 déc.; fallback synthétique visible. |
...
| Swarm Inference Backend | Distributed model execution (Python) | 🔄 | Abstraction InferenceProvider prioritaire; stub OG + `inferenceProofHash` côté audit + pack v2. |
...
| v3 (plan) | v2 + valeurs enrichies (momentumShortMinusLong, priceChangePct, unrealizedPnLToken, abnormalTransferFlag) + `priceQuantized` (oracle Switchboard) | Ajout features étendues & prix quantifié | Étendre signal décision tout en restant déterministe; quantization prix évite micro drift | À implémenter (post abstraction PriceProvider) | +4 (incluant priceQuantized) |

Notes:
1. Double publication: `featureHash` (v1) + `featureHashV2` (stable). Comparaisons ⇒ utiliser v2.
2. Migration v3 via constante `FEATURE_SET_VERSION=3` + nouveau `featureHashV3` (double émission temporaire) incluant `momentumShortMinusLong` et `priceQuantized`.
...
---
## 4. Modèle / Moteur Déterministe (TypeScript v1) & Swarm Stub
| Élément | Détail |
|---------|--------|
...
### Swarm Inference (stub OG)
- Provider `opengradient-stub` appelé via `INFERENCE_PROVIDER=opengradient_stub`.
- Produit un blob d’inférence canonique → `inferenceProofHash` ajouté dans `ai_decision`.
- Proof Pack v2 inclut `inference.json`; vérification re-hash côté CLI.
