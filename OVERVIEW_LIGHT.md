...
## 6. Ce qui arrive ensuite (étapes courtes valeur)
Ordre volontairement minimal:
1. Corriger warnings TypeScript viem (bruit technique).
2. Ajouter "donation_intent" (intention caritative) → montre extensibilité sociale.
3. Intégrer stub OpenGradient (provider alternatif) → prouver que le moteur peut basculer d'un provider à un autre avec toujours de la preuve.

(Plus tard seulement: oracle externe Switchboard, features v3 étendues.)
